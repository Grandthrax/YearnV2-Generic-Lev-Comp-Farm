...
def test_add_dai(
    ...
    strategy = live_strategy_dai_030_3
    vault = live_vault_dai_030
    ...
    #i = 0
    #while strategy.estimatedTotalAssets() > 40*1e18:
    #    strategy.harvest({'from':gov})
    #    stateOfStrat(strategy, dai, comp)
    #    i = i + 1
    #    print(i)
    ...
    startingProfit = live_vault_dai_030.strategies(strategy)[6]
    waitTime = 100
    chain.mine(waitTime)
    chain.sleep(1)
    strategy.harvest({'from':gov})
    profit = live_vault_dai_030.strategies(strategy)[6] - startingProfit
    ...
    stateOfStrat(strategy, dai, comp)
    genericStateOfStrat(strategy, dai, live_vault_dai_030)

    #emergency exit
    live_vault_dai_030.updateStrategyDebtRatio(strategy, 0, {'from':gov})
...
def test_screenshot(
    ...
    #strategy.setDyDx(False, {'from':samdev})
    strategy.harvest({"from": strategist})
    #strategy.setDyDx(True, {'from':samdev})
    strategy.harvest({"from": strategist})
